...
namespace
{
template <typename Default, typename Base> std::shared_ptr<Base> orDefault(std::shared_ptr<Base> p)
{
    return p ? std::move(p) : std::make_shared<Default>();
}

// Effective wind at the ball: the launch wind above hWind, otherwise none.
Vector3D effectiveWind(const ShotPhysicsContext &physicsVars, const AtmosphericData &atmos,
                       float heightZ)
{
...
AerodynamicState buildAeroState(const BallState &state, const ShotPhysicsContext &physicsVars,
                                const AtmosphericData &atmos, float ballRadius)
{
    const Vector3D wind = effectiveWind(physicsVars, atmos, state.position[2]);
    return AerodynamicState{.velocity = state.velocity,
...
Vector3D flightAcceleration(const AerodynamicModel &model, const AerodynamicState &aeroState,
                            float gravity)
{
...
AccelerationField makeAccelField(std::shared_ptr<AerodynamicModel> model,
                                 ...
                                 const AtmosphericData &atmos, float ballRadius, float gravity,
                                 std::shared_ptr<Vector3D> stepSpin)
{
    return [model = std::move(model), &physicsVars, atmos, ballRadius, gravity,
        ...
        return flightAcceleration(*model, buildAeroState(trial, physicsVars, atmos, ballRadius),
                                  gravity);
    };
}
} // namespace
...
AerialPhase::AerialPhase(ShotPhysicsContext &physicsVars, [[maybe_unused]] const LaunchData &launch,
...
void AerialPhase::initialize(BallState &state)
{
    ...
    v = std::sqrt(state.velocity[0] * state.velocity[0] + state.velocity[1] * state.velocity[1] +
...
BouncePhase::BouncePhase(ShotPhysicsContext &physicsVars, [[maybe_unused]] const LaunchData &launch,
                         const AtmosphericData &atmos, std::shared_ptr<TerrainInterface> terrain,
                         std::shared_ptr<AerodynamicModel> aeroModel,
                         std::shared_ptr<BounceModel> bounceModel, const BallProperties &ball,
                         float gravity, std::shared_ptr<Integrator> integrator)
    : physicsVars(physicsVars)
    , atmos(atmos)
    , terrain(terrain)
...
void BouncePhase::calculateStep(BallState &state, float dt)
{
    ...
    const GroundSurface &surface =
        terrain->getSurfaceProperties(state.position[0], state.position[1]);
    ...
    {
        BounceState bounceState{state.velocity, surfaceNormal, state.spinVector, ballRadius};
...
RollPhase::RollPhase(std::shared_ptr<TerrainInterface> terrain, std::shared_ptr<RollModel> model,
...
void RollPhase::calculateStep(BallState &state, float dt)
{
    ...
    const RollState rollState{state.position, state.velocity, state.spinVector,
                              surfaceNormal,  ballRadius,     dt,
                              terrain.get()};
