...
FlightSimulator::FlightSimulator(const LaunchData &launch, const AtmosphericData &atmos,
                                 const GroundSurface &ground,
                                 std::shared_ptr<AerodynamicModel> aeroModel,
                                 std::shared_ptr<BounceModel> bounceModel,
                                 std::shared_ptr<RollModel> rollModel, const BallProperties &ball,
    ...
    , aerialPhase(physicsVars_, launch, atmos, terrainStorage_, aeroModel, ball, gravity,
                  integrator)
...
void FlightSimulator::initializeFromLaunch(const LaunchData &launch)
{
    ...
    const float launchTerrainHeight = terrainStorage_->getHeight(launch.startX, launch.startY);
    startPosition_ = Vector3D{launch.startX, launch.startY, launchTerrainHeight + launch.startZ};
    const Vector3D &startPos = startPosition_;

    state = BallState::fromLaunchParameters(v0_fps, launch.launchAngleDeg, launch.directionDeg,
                                            startPos, gravity_, physicsVars_.getW());
    ...
    for (long step = 0; currentPhase != Phase::Complete; ++step)
    {
        if (step >= maxSteps)
        {
            throw std::runtime_error(
                ...
                std::to_string(physics_constants::MAX_SIMULATION_TIME) + "s (stuck in " +
                getCurrentPhaseName() +
...
LandingResult FlightSimulator::getLandingResult() const
{
    ...
    result.zF = (state.position[2] - finalTerrainHeight) / physics_constants::YARDS_TO_FEET;
    result.timeOfFlight = state.currentTime;
    result.bearing = std::atan2(relative[0], relative[1]) * 180.0F / physics_constants::PI;
