...
{
  public:
    ShotPhysicsContext(const LaunchData &launch, const AtmosphericData &atmos,
    ...
    [[nodiscard]] auto getRhoMetric() const -> float
    {
        return rhoMetric;
    }
    /// Lumped aerodynamic force coefficient (air density × cross-section / mass); c0·v² yields ft/s².
    [[nodiscard]] auto getC0() const -> float
    {
        return c0;
    }
    /// Launch speed magnitude, ft/s.
    [[nodiscard]] auto getV0() const -> float
    {
        return v0_magnitude;
    }
    /// Launch velocity vector, ft/s.
    [[nodiscard]] auto getV0Vector() const -> Vector3D
    {
        return v0;
    }
    /// Spin vector, rad/s (direction = spin axis).
    [[nodiscard]] auto getW() const -> Vector3D
    {
        return w;
    }
    /// Spin magnitude, rad/s.
    [[nodiscard]] auto getOmega() const -> float
    {
        return omega;
    }
    /// Surface speed r·ω, ft/s.
    [[nodiscard]] auto getROmega() const -> float
    {
        return rOmega;
    }
    /// Air temperature, °C.
    [[nodiscard]] auto getTempC() const -> float
    {
        return tempC;
    }
    /// Elevation above sea level, m.
    [[nodiscard]] auto getElevationM() const -> float
    {
        return elevationM;
    }
    /// Wind velocity vector, ft/s.
    [[nodiscard]] auto getVw() const -> Vector3D
    {
        return vw;
    }
    /// Saturation vapor pressure, mmHg.
    [[nodiscard]] auto getSVP() const -> float
    {
        return SVP;
    }
    ...
    [[nodiscard]] auto getRe100() const -> float
    {
        return Re100;
    }
    /// Dynamic viscosity of air, Pa·s (= kg/(m·s)).
    [[nodiscard]] auto getAirViscosity() const -> float
    {
        return airViscosity;
    }
    /// Air temperature, K.
    ...
    [[nodiscard]] auto getRelHumidity() const -> float
    {
        return atmos.relHumidity;
    }
