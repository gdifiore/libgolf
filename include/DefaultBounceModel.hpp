...
class DefaultBounceModel : public BounceModel
{
  public:
                                             ...
                                             const GroundSurface &surface) const override
    {
        ...
        const float speedNormalMs = std::abs(vDotN) * physics_constants::FEET_TO_METERS;

        const float effectiveCor =
            surface.restitution *
        ...
        const bool energeticImpact = impactSpeed >= MIN_PENNER_BOUNCE_SPEED_FT_PER_S;

        Vector3D vTangentAfter{};

        if (steepImpact && energeticImpact && tangentMag > physics_constants::MIN_SPEED)
        {
            ...
            const float retention =
                BOUNCE_RETENTION_BASE * std::clamp(1.0F - spinRpm / BOUNCE_RETENTION_RPM_NORM,
                                                   BOUNCE_RETENTION_FLOOR, 1.0F);
            const float spinbackTerm = (2.0F * state.ballRadius * backspinScalar) / 7.0F;
        ...
        return BounceResult{vNormalAfter + vTangentAfter, state.spinVector * surface.spinRetention};
    }
  ...
  private:
    static float corVelocityScale(float speedNormalMs)
    {
        ...
        {
            return BOUNCE_COR_VEL_MID_SCALE * (speedNormalMs / BOUNCE_COR_VEL_LOW_MS);
        }
        if (speedNormalMs < BOUNCE_COR_VEL_HIGH_MS)
        {
            const float t = (speedNormalMs - BOUNCE_COR_VEL_LOW_MS) /
                            (BOUNCE_COR_VEL_HIGH_MS - BOUNCE_COR_VEL_LOW_MS);
            return BOUNCE_COR_VEL_MID_SCALE + (1.0F - BOUNCE_COR_VEL_MID_SCALE) * t;
        }
        return 1.0F;
    }
    ...
    static float corMaxReduction(float spinRpm)
    {
        ...
        {
            return (spinRpm / BOUNCE_COR_SPIN_KNEE_RPM) * BOUNCE_COR_SPIN_LOW_MAX_REDUCTION;
        }
        const float excess = spinRpm - BOUNCE_COR_SPIN_KNEE_RPM;
        const float t = std::min(excess / BOUNCE_COR_SPIN_HIGH_BAND_RPM, 1.0F);
