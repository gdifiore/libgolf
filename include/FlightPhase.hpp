...
{
  public:
    AerialPhase(ShotPhysicsContext &physicsVars, const LaunchData &launch,
                const AtmosphericData &atmos, std::shared_ptr<TerrainInterface> terrain,
                std::shared_ptr<AerodynamicModel> model = nullptr, const BallProperties &ball = {},
    ...
    [[nodiscard]] auto getV() const -> float
    {
        return v;
    }
    [[nodiscard]] auto getVMph() const -> float
    {
        return vMph;
    }
    [[nodiscard]] auto getTau() const -> float
    {
        return tau;
    }
    [[nodiscard]] auto getVw() const -> float
    {
        return vw;
    }
    [[nodiscard]] auto getVwMph() const -> float
    {
        return vwMph;
    }
...
{
  public:
    BouncePhase(ShotPhysicsContext &physicsVars, const LaunchData &launch,
                const AtmosphericData &atmos, std::shared_ptr<TerrainInterface> terrain,
                std::shared_ptr<AerodynamicModel> aeroModel = nullptr,
                std::shared_ptr<BounceModel> bounceModel = nullptr, const BallProperties &ball = {},
                float gravity = physics_constants::GRAVITY_FT_PER_S2,
                std::shared_ptr<Integrator> integrator = nullptr);
...
class RollPhase : public FlightPhase
{
  public:
    explicit RollPhase(std::shared_ptr<TerrainInterface> terrain,
                       std::shared_ptr<RollModel> model = nullptr, const BallProperties &ball = {});
