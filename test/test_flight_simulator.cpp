...
{
  public:
    explicit SteepDownhillTerrain(float slope)
        : slope_(slope)
    {
    ...
    [[nodiscard]] auto getNormal([[maybe_unused]] float x,
                                 [[maybe_unused]] float y) const -> Vector3D override
    {
    ...
    [[nodiscard]] auto getSurfaceProperties(
        [[maybe_unused]] float x, [[maybe_unused]] float y) const -> const GroundSurface & override
    {
        return surface_;
    }
...
{
  public:
    [[nodiscard]] Vector3D computeAcceleration(
        [[maybe_unused]] const AerodynamicState &s) const override
    {
        const float nan = std::numeric_limits<float>::quiet_NaN();
        return {nan, nan, nan};
    }

    [[nodiscard]] float computeSpinDecayTau(
        [[maybe_unused]] const AerodynamicState &s) const override
    {
        return 1e6F;
    }
...
{
  public:
    explicit CountingIntegrator(std::shared_ptr<int> calls)
...
TEST_F(FlightSimulatorTest, ExplicitDefaultIntegratorMatchesImplicit)
{
    ...
    FlightSimulator explicitSim(ball, atmos, ground,
                                ...
                                /*ball*/ BallProperties{}, physics_constants::GRAVITY_FT_PER_S2,
...
TEST_F(FlightSimulatorTest, ProducesReasonableTrajectory)
{
    ...
    float finalDistance = std::sqrt(finalState.position[0] * finalState.position[0] +
...
TEST_F(FlightSimulatorTest, HandlesNonZeroGroundHeight)
{
    ...
    float finalSpeed = std::sqrt(finalState.velocity[0] * finalState.velocity[0] +
