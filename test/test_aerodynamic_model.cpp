...
TEST_F(DefaultModelTest, ClLowReRamp)
{
    ...
    double expected =
        std::clamp(cl50 * 0.5, 0.0, static_cast<double>(DefaultAerodynamicModel::CL_MAX_BASE));
...
TEST_F(DefaultModelTest, ClBetweenBinsLerp)
{
    ...
    double expected = std::clamp(0.5 * (cl50 + cl60), 0.0,
                                 static_cast<double>(DefaultAerodynamicModel::CL_MAX_BASE));
...
{
  public:
    explicit ConstantTauModel(float tau)
        : tau_(tau)
    {
    }
    Vector3D computeAcceleration(const AerodynamicState &) const override
